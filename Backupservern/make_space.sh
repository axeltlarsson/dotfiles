...
# such as /media/axel and NOT /dev/sda3
disk_usage() {
...
delete_oldest_dir() {
	...
	IFS= read -r -d $'\0' file < <(find ./test -maxdepth 1 ! -path ./test  -type d -printf '%p\0' | sort -z)
	#rm -rf "$file"
	find "$file" # see that it is the current file
}

target=0.85
disk_usage "/media/axel"
